...
const getOrders = async (req: Request, res: Response) => {
    try {
        ...
        return res.status(200).json({ message: "Orders fetched successfully", data: orderData });
    } catch (error) {
        console.log(error);
...
const updateOrder = async (req: Request, res: Response) => {
    try {
        ...
        const updateOrder = await Order.findOneAndUpdate({ _id: orderId }, updateOrderData);
        return res.status(200).json({ message: "Order updated successfully", data: updateOrder });
    } catch (error) {
        console.log(error);
